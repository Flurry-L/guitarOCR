...
审核日期：2026-09-25。对象是工作目录中的重构版本，审核基线提交为 `84645ea`。审核时已有的大量未提交迁移得到保留；后续按处理流程与工作台、模型、安装与 CI、文档四组整理为本地提交，并为新检出验收发现的导入状态并发问题补充独立修复提交。没有改写既有历史或公开发布。
...
| `webapp/app.py` | 会话响应原先引用共享任务字典，后台导入可能在响应序列化之前将状态改成完成，形成“没有页面但任务已完成”的不一致响应。现在在锁内复制任务状态；确定性回归验证后台完成不会改变已生成的响应快照。 |
...
| --- | --- |
| 标准 Ruff 检查 | 通过 |
| 原有核心 / API / 数据 / 安装器测试 | 基线 37 项通过；新增审核与导入状态快照回归后共 45 项通过 |
| 独立轻量环境 | 按更新后的 uv.lock 新建，仅安装 webui / dev，确认没有 Torch；最终提交的新检出中 45 项全部通过 |
...
新增回归覆盖 Host 与 Origin、畸形 ZIP 与路径别名、伪装图片、基座与 LoRA 切换、小节线行为、发布包内容和导入任务状态快照。没有运行完整训练、Windows / GP8 验收或全部第三方二进制的安全分析。
