...
| 原位置 | 当前归属 |
| --- | --- |
| 整谱识别文件的定位、裁图部分 | `layout/geometry.py`、`layout/crops.py` |
...
| 原生导出入口及内部实现 | `datagen/export_scores.py`、`datagen/native/` |
| GP5 导出大文件 | `gp5_export/writer.py`、`fingering.py`、`effects.py` |
| 集中的训练／评测脚本和配置 | 对应步骤内的 `train.py`、`evaluate.py`、`configs/` |

所有模块共用根目录的 `pyproject.toml` 和 `uv.lock`。Python 模块路径已迁移，旧的 `python -m guitarocr...` 命令请改用各步骤入口；`guitarocr-gp` 命令保留，安装更新后指向 `pipeline.run:main`。
...
GP8 导出器既支持 `python -m datagen.export_scores`，也支持 `python datagen/export_scores.py`。后一种入口用于 Wine 内的 Python，原生 DLL 仍从 `datagen/native-bin/` 加载。完整 C++ 源码、符号定义和构建脚本已迁入 `datagen/native-source/`，构建命令见 [native-build.md](native-build.md)。

## 交互式编辑

`webapp/workflow.py` 管理持久化会话，调用上述阶段；`layout/edit.py` 验证原图坐标并重新裁图。Web 请求和单队列在 `webapp/app.py`，前端文件在 `webapp/static/`。每次修改产生新阶段目录，更新会话指针并失效下游引用。`review_measures` 保存需人工确认的占位小节；小节记录包含 `needs_review`、`fallback_reason` 和 `manually_edited`。
