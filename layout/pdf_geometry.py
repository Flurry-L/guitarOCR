...
def _horizontal_rows(page: Any) -> dict[float, list[tuple[float, float]]]:
    drawings = page.line_paths()
    if not drawings:
        return {}

    def horizontal_count(drawing) -> int:
        count = 0
        for start, end in drawing:
            if abs(end[0] - start[0]) > 20 and abs(end[1] - start[1]) < 0.15:
                count += 1
        return count
    ...
    for start, end in drawing:
        ...
        if abs(end[0] - start[0]) <= 1.0 or abs(end[1] - start[1]) >= 0.15:
            continue
        y = round((float(start[1]) + float(end[1])) / 2.0, 2)
        rows[y].append((min(float(start[0]), float(end[0])), max(float(start[0]), float(end[0]))))
    return {
...
def extract_pdf_vector_measure_boxes(
    ...
    result = {}
    with open_pdf(Path(pdf)) as document:
