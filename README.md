...
支持六线谱 TAB、五线谱和五线谱＋TAB，默认自动判断谱面类型。当前模型主要适用于规则排版的吉他单轨谱，手写、透视照片和复杂总谱尚需更多验证。识别结果需要对照原谱校对，效果和已知错误见[评测报告](docs/model-evaluation.md)。
...
NVIDIA 驱动 580 或更新版本可自动启用 GPU，否则使用 CPU。无需提前安装 Python 或 CUDA Toolkit。Linux CPU 和 H100 已实测；Windows 提供启动脚本，完整安装仍需实机验收。详细参数和手动安装见[安装说明](docs/setup.md)，安装失败见[故障排查](docs/troubleshooting.md)。
...
服务端首页无需登录，可直接使用浏览器 CPU，通过 WASM 推理。登录后可使用服务器 GPU、保存任务和结果；GPU 任务在关闭网页后继续运行，浏览器任务会暂停。用户可自行注册，管理员可管理用户、查看用量和队列，并在网页安装仓库更新。
...
运行时有三项任务模型：一套 PP-DocLayout 定位并判断谱面类型，两套 GLM-OCR LoRA 分别读取谱面信息和小节，共用一个基座。模型用途、下载与校验见[模型说明](weights/README.md)。

| 需要做什么 | 文档 |
| --- | --- |
| 修改代码、检查和打包 | [参与开发](CONTRIBUTING.md) |
| 编辑或导入小节文本 | [格式参考](docs/score-text.md) |
| 从 Guitar Pro 生成训练数据 | [数据生产](docs/data.md) |
| 训练和评测模型 | [训练说明](docs/training.md) |
| 查看模型效果和已知错误 | [评测报告](docs/model-evaluation.md) |

项目许可证尚未选定。第三方组件和模型的授权说明见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。
