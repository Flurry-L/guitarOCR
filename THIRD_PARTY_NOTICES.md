...
| --- | --- | --- |
| [PyMuPDF / MuPDF](https://pymupdf.readthedocs.io/en/latest/about.html#license-and-copyright) | GNU AGPL-3.0 或 Artifex 商业许可 | 本项目直接导入并使用该组件；选择项目及组合软件的许可时，需处理 AGPL 的适用义务或取得适用商业授权。 |
...
- `examples/` 标注为项目自编示例；项目所有者仍需明确示例图、PDF、小节文本和 GP5 使用哪一许可证。

## GP8 原生数据工具

`datagen/native-source/`、`datagen/native-bin/` 包含与 Guitar Pro 内部 ABI 配合的自定义代码、声明和 DLL。来源及构建状态见 [原生构建文档](docs/native-build.md)。

Guitar Pro 是 Arobas Music 的专有软件；本仓库不提供其安装程序或运行库。发布前需确认迁入代码的著作权归属、声明文件来源，以及这些工具的使用和再分发是否符合适用条款。Qt 使用版本、许可选项及链接方式也需结合真实构建核对。当前 DLL 已由仓库源码交叉编译并在 GP8＋Wine 中验收；构建通过不解决代码与接口的授权问题。

发布待办见 [开源审核记录](docs/open-source-audit.md)。
