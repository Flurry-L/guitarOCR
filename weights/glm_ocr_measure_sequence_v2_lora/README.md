...
- 适配器权重约 31.5 MB。
- 数据：GP 源谱解析标签与官方 GP8 渲染的小节裁图；通过源谱分组隔离训练、验证、测试，补充技巧难例。当前数据管线支持导出纯 TAB、纯五线谱和混合谱；本次版面模型更新未改动此 LoRA。
- 本版适配器的完整历史训练清单、样本量和 checkpoint 步数尚未随可发布记录保留；不要将新的数据构建结果描述为该检查点的精确训练快照。后续训练应保存 source_catalog 哈希与训练记录。
