...
def _gp8_samples(
    ...
    for document in sorted((export_root / "native-export" / "documents").glob("tab-*")):
        ...
        with open_pdf(tracks[0].with_name("score.pdf")) as pdf:
                        ...
                        if any(value is not None for value in target.values()):
                            printed = page.text(
                                (
                                    ...
                                    0,
                                    page.width,
                                    header_bottom / image.height * page.height,
                                )
                            )
                            if any(
    ...
    for track in tracks:
        ...
        if first:
            ...
            if measures:
                ...
                target = _header_target(score)
                if any(value is not None for value in target.values()):
                    with open_pdf(folder / "score.pdf") as pdf:
                        header_bottom_points = (
                            header_bottom
                            / float(first["image_size"][1])
                            * pdf[0].height
                        )
                        printed = pdf[0].text((0, 0, pdf[0].width, header_bottom_points))
                    if any(
                        _printed_text(value) not in _printed_text(printed)
                        for value in target.values()
                        if value
