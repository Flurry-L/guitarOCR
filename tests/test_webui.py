...
class WebWorkflowTest(unittest.TestCase):
    ...
    def test_multipage_pdf_keeps_filename_and_all_pages(self):
        ...
        for i in range(4):
            canvas.setPageSize((160 + i * 10, 120))
            canvas.drawString(10, 90, f"Page {i + 1}")
            canvas.showPage()
        canvas.save()
...
class InputPagesTest(unittest.TestCase):
    ...
    def test_pdf_types_input_order_exif_and_multiframe(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            canvas = Canvas(str(raster), pagesize=(120, 80))
            canvas.drawImage(str(bitmap), 0, 0, 120, 80)
            canvas.save()
            vector = root / "vector.pdf"
            canvas = Canvas(str(vector), pagesize=(120, 80))
            for y in range(10, 70, 10):
                canvas.line(10, y, 110, y)
            canvas.save()
