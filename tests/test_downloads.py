...
class PackageSourceTest(unittest.TestCase):
    ...
    def test_model_mirror_uses_a_pinned_revision(self):
        base = json.loads((launcher.ROOT / "weights/manifest.json").read_text())["base_model"]
        for source in base["download_sources"]:
            self.assertRegex(source, r"/resolve/[0-9a-f]{40}$")
